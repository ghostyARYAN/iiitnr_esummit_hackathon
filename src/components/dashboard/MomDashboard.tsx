...
import {
  FileText, CheckCircle, Clock, BookOpen, ArrowRight, TrendingUp, CalendarDays, Search, SlidersHorizontal
} from "lucide-react";
...
export default function MomDashboard() {
  ...
  const [stats, setStats] = useState({
    pending_mom: 0, mom_generated: 0, finalized: 0
  });
    ...
    const fetchMomData = async () => {
      ...
      if (allApps) {
        const counts = allApps.reduce((acc, app) => {
          if (app.status === 'referred') acc.pending_mom++; // Assuming 'referred' means ready for MoM
          else if (app.status === 'mom_generated') acc.mom_generated++;
          else if (app.status === 'finalized') acc.finalized++;
          return acc;
        }, { pending_mom: 0, mom_generated: 0, finalized: 0 });
        setStats(counts);
      }

      // Fetch pending MoM applications
      const { data: apps } = await supabase
        .from("applications")
        .select("id, project_name, status, sector_id, created_at, category")
        .eq('status', 'referred') // Pending MoM
  ...
  const cards = [
    {
      title: "Pending MoM",
      value: stats.pending_mom,
      icon: Clock,
      trend: "Needs Action",
      trendUp: false,
      subText: "Awaiting Minutes generation",
    },
    {
      title: "MoM Generated",
      value: stats.mom_generated,
      icon: FileText,
      trend: "Completed",
      trendUp: true,
      subText: "Minutes recorded",
    },
    {
      title: "Finalized",
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {t("Manage Minutes of Meetings and documentation")}
          </p>
        </div>
            ...
            <Card key={card.title} className="rounded-2xl border-border/40 shadow-sm hover:shadow-md transition-all duration-300 group">
              <CardContent className="p-6">
                ...
                <div className="flex items-baseline gap-3">
                  ...
                  <span className={`text-xs font-semibold flex items-center gap-0.5 px-2 py-0.5 rounded-full ${card.trend === "Needs Action"
      ...
      <Card className="rounded-2xl border-border/40 shadow-sm">
        <CardHeader className="pb-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <CardTitle className="text-base font-semibold">{t("Applications Awaiting MoM")}</CardTitle>
        ...
        <CardContent className="pt-0">
          {loading ? (
            ...
            <>
              ...
              <div className="flex justify-end mt-4">
                <Button variant="ghost" size="sm" className="gap-1 text-primary hover:text-primary/80 hover:bg-primary/5 rounded-lg" asChild>
                  <Link to="/dashboard/mom">
                    {t("View All Tasks")} <ArrowRight className="h-3.5 w-3.5" />
                  </Link>
                </Button>
              </div>
            </>
            ...
            <div className="text-center py-12 text-muted-foreground">
              <BookOpen className="mx-auto h-10 w-10 text-emerald-500/30 mb-3" />
              <p className="font-medium">{t("All caught up! No pending MoM generation.")}</p>
            </div>
          )}
        </CardContent>
      </Card>
